...
class Room:
    ...
    A Room has the following variables:
        characters         -   A set of Characters in the Room
        items              -   A set of Items in the Room
        accessible_rooms   -   A set of other Rooms that are accessible from the Room
        locked             -   Boolean representing if the location is locked or not (from all directions).

    A Room has the following methods:
        describe(self)                  -   Print description of Room and all Characters and Items
        toggle_lock(self, key)          -   Toggle the Room.locked value between True/False.
                                            Must use a Key.
        get_room(self, name)            -   Returns the Room object with name name if the name
                                            is part of self.accessible_room
        get_item(self, name)            -   Returns the Item with name name if name is in self.items
        add_accessible_room(self, room) -   Add a new room to the self.accessible_rooms set.

    """
    def __init__(self, name, description, characters, items, accessible_rooms=set(), locked=False):
    ...
    def describe(self):
        ...
        if len(self.characters) > 0:
            ...
            for character in self.characters:
                print("\t {}:\t {}".format(character.name, character.description))
    ...
    def get_room(self, name):
        ...
        Input
            name    -   A name (string). Check if any Room in self.accessible_rooms
                        has this name.

        Output
            room    -   If there is an accessible room with the input name return the Room object 
    ...
    def get_character(self, name):
        ...
        Input
            name    -   A name (string). Check if any Character in self.characters
                        has this name.

        Output
            character -   If the Character is here, return the Character object
                          otherwise, returns None
        """
        for character in self.characters:
    ...
    def get_item(self, name):
        """ Return an item that is present at this location.
        Item will be removed from self.locations and returned.

        Input
            name    -   A name (string). Check if any Item in self.items
                       has this name.
        Output
            item    -   The Item object with the input name if it is in the Room.
                        Otherwise, returns None.
        """
    ...
    def add_accessible_room(self, room):
...
class Character:
    ...
    """
    def __init__(self, name, description, message, other_message=""):
        """ Initialize a Character 
        with a name, description, messages, and an empty backpack """
        self.name = name
        self.description = description
...
class Hero(Character):
    ...
    The Hero is able to:
        explore(self)        -   Look around.
        go_to(self, name)    -   Updated self.location to be Room with name "name" only if 
                                 name is in the self.location.accessible_rooms set.
        talk_to(self, name)  -   Talk to a Character with name "name" only if that Character 
                                 is in the self.location.characters set.
        pick_up(self, name)  -   Pick up an Item if it is in self.location.items
        unlock(self, name)   -   Unlock a room only a key exists in self.backpack
                                AND the location with name is accessible from self.location.
    
    Example usage:

        # Create a hero object
        alice = Hero("Alice", "Game hero", staircase, "Hello, I'm looking for the exit")
    ...
    """
    def __init__(self, name, description, message, other_message, location, backpack=set()):
        """Initialize the Hero"""
    ...
    def explore(self):
        ...
        print("You are currently in {}:\n\t{}.".format(self.location.name, self.location.description))
        self.location.describe()

    def go_to(self, name):
        """ Move your hero around.
        If a Room named "name" exists in self.location.accessible_rooms, move Hero by updated
        self.location to this room.
        
        Input
            name    -   A name (string) of a Room that the Hero is to move to.

        """
        # Check if any Room with "name" is accesible from the current Room (self.location)
        if self.location.get_room(name):
            if name == "portal":
                print("You walk through the magic portal and somehow you ended up back in room107.")
                ...
                if not new_room.locked:
                    print("You move from the {} to the {}.".format(self.location.name, name))
                    self.location = new_room
                else:
                    print("You can't get into {}. It's locked. Do you have a key?".format(name))

    def talk_to(self, name):
        """Talk to character if character is at hero's current place.

        Input
            name    -   A name (string) of a Character. If there is a Character
                        with this name in self.location.characters, talk to them.
    ...
    def pick_up(self, name):
        ...
        Input
            name    -   A name (string) of an Item.
        """
        # TODO replace with your code
        print("****** Hero.pick_up is not yet implemented ******")

    def rummage(self):
        """ Print each item in self.backpack with its description and return a list of item names. """
        # TODO replace with your code
        print("****** Hero.rummage is not yet implemented ******")

    def unlock(self, name):
        """If hero has a key, toggle a room lock only if the room is accessible from current location.
        
        Method will check if there is a Room in self.accessible_rooms with the input name
        AND self.backpack contains at least on Key object before 
        using the Key to unlock the Room. 

        Input
            name    -   A name (string) of a Room object. """
        # TODO replace with your code
...
def print_guide():
    ...
    print("\t\'rummage\'\t\t Check the belongings you currently have in your backpack.")
    print("\t\'talk_to [name]\t\t Talk to another Chacater in the same room as you.")
    print("\t\'go_to [name]\'\t\t Walk through a room accessible from the current room you are in.")
    print("\t\'pick_up [name]\'\t Pick up an item that is in the room.")
    print("\t\'unlock [name]\'\t\t Unlock a nearby room. Will only work if you currently have a Key item.")
    print("")
...
if __name__ == "__main__":
    ...
    hero = Hero(hero_name, "Hero of the game (You!)", "I'm trying to get out of here!", "Can you help me find the exit?", room107)
